...
**Last Updated:** 2026-09-17 (Canonical academy-time scheduling hardening: device/server-independent display, input parsing, day/month filters, recurring-rule defaults, booking-conflict parity, and live wizard QA)
...
**Tests:** ✅ `npm test` (server, jest) — **588/588 passing, 53 suites**; `npm test` (client, vitest) — **90/90 passing, 7 files**. Scheduling coverage includes academy wall-time parsing/formatting, day/month boundaries, recurring-session generation/synchronization/deduplication, report tracking, and finish/reschedule controller workflows.
